...
public class PaymentCommonService {
    ...
    public Payment getPayment(String paymentId) {
        return paymentRepository.findPaymentByOrderId(paymentId).orElseThrow(() -> new PaymentException(ErrorCode.DATA_NOT_FOUND));
    }

    @Transactional(readOnly = true)
    public Payment getPayment(Long paymentId) {
        return paymentRepository.findById(paymentId).orElseThrow(() -> new PaymentException(ErrorCode.DATA_NOT_FOUND));
    }
}
