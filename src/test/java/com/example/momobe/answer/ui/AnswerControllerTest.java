...
import static org.aspectj.apache.bcel.generic.Type.STRING;
...
@Import({SecurityTestConfig.class, AnswerMapper.class})
...
class AnswerControllerTest {
    ...
    @Test
    @DisplayName("답변 작성 시 유효성 검사에 실패할 경우 400 반환")
    void postQuestion_failed_400() throws Exception {
        // given
        AnswerDto request = AnswerDto.builder().content(" ").build();
        String json = objectMapper.writeValueAsString(request);
        // when
        ResultActions perform = mockMvc.perform(post("/meetings/{meeting-id}/questions/{question-id}/answers", 1L, 1L)
